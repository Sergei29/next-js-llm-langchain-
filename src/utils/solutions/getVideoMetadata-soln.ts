...
export default async function getVideoMetaData(id: string) {
  ...
  const url = `https://www.googleapis.com/youtube/v3/videos?id=${id}&key=${process.env.GOOGLE_API_KEY}&part=snippet,contentDetails,statistics,status`

  const [data, error] = await fetchJsonData<Record<string, any>>(url)
  if (error || !data) {
